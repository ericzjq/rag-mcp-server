...
本项目不仅是一个功能完备的智能问答框架，更是一个专为 **RAG 技术学习** 设计的实战平台：

#### 1️⃣ 实战驱动学习 (Learn by Doing)

项目架构本身就是 RAG 理论知识的"**活体答案**"。将经典理论直接融入代码设计，通过动手实践来巩固理论知识：
...
| C11  | BM25Indexer（倒排索引+IDF计算）                 | [x] | 2026-03-02 | IDF、倒排索引、save/load、query 稳定 top ids |
...
| D7   | 脚本入口 query.py（查询可用）                  | [x] | 2026-03-02 |     |
...
| E5   | get_document_summary Tool  | [x] | 2026-03-02 | doc_id→title/summary/tags、metadata 文件或 _store、不存在返回 structuredContent.error |
| E6   | 多模态返回组装（Text + Image）      | [x] | 2026-03-02 | multimodal_assembler、metadata.images→ImageContent、mimeType/base64、query_knowledge_hub 接入 |
...
| 任务编号 | 任务名称                                        | 状态  | 完成日期 | 备注  |
| ---- | ------------------------------------------- | --- | ---- | --- |
| F1   | TraceContext 增强（finish + 耗时统计 + trace_type） | [x] | 2026-03-02 | trace_type/finish/elapsed_ms/to_dict、TraceCollector.collect、可 JSON 序列化 |
...
| I3   | 完善 README（运行说明 + MCP + Dashboard） | [x] | 2026-03-03 | 快速开始/配置说明/MCP 示例/Dashboard/测试/FAQ/全链路验收说明 |
| I4   | 清理接口一致性（契约测试补齐）                   | [x] | 2026-03-03 | test_vector_store_contract delete_by_metadata；reranker 空候选；evaluator 空列表边界 |
| I5   | 全链路 E2E 验收                        | [x] | 2026-03-03 | README 验收步骤、sample_documents/simple.pdf、pytest -q 全绿 |
| I6   | 项目总结（INTERVIEW.md）                 | [x] | 2026-03-02 | 项目职责+主要工作三部分+JD 自检 |
...
- **状态**：已完成（2025-03-01）。备注：ollama_llm.py、默认 base_url、工厂注册、mock 测试与可读错误。
