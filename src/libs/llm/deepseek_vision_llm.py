"""
DeepSeek Vision LLM：通过 DeepSeek API（OpenAI 兼容）进行图像理解。
需 DeepSeek 开放视觉模型接口；调用方式与 Azure Vision 一致（chat completions + image_url）。
"""
...
class DeepSeekVisionLLM(BaseVisionLLM):
    """DeepSeek Vision（OpenAI 兼容多模态接口）实现。"""
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        self._model = (self._vl.model or "").strip() or "deepseek-chat"
