...
def _validate_messages(messages: List[Dict[str, Any]]) -> None:
    ...
    for i, m in enumerate(messages):
        if not isinstance(m, dict):
            raise ValueError(f"{PROVIDER_NAME}: messages[{i}] 必须为 dict，当前为 {type(m).__name__}")
...
class OpenAILLM(BaseLLM):
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        self._api_key = (settings.llm.api_key or "").strip() or None  # 可后续从 env 读取
        self._base_url = (settings.llm.base_url or "").strip() or None
        self._client = None  # 延迟初始化以便 mock
    ...
    def _get_client(self):
        if self._client is None:
            try:
                ...
                raise RuntimeError(f"{PROVIDER_NAME}: 需要 openai 包，请 pip install openai") from e
            self._client = OpenAI(
