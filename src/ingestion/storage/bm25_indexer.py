...
class BM25Indexer:
    ...
    def query_with_scores(self, terms: List[str], top_k: int = 10) -> List[Tuple[str, float]]:
        """与 query 相同打分逻辑，返回 (chunk_id, score) 列表，供 SparseRetriever 合并 text/metadata。"""
        if not terms or self._n == 0:
            return []
        scores: Dict[str, float] = {}
        for term in terms:
            ...
            for p in info["postings"]:
                ...
                score = idf * (tf * (K1 + 1)) / denom
                scores[cid] = scores.get(cid, 0.0) + score
        sorted_pairs = sorted(scores.items(), key=lambda x: (-x[1], x[0]))
    ...
    def remove_document(self, chunk_ids: List[str]) -> int:
        ...
        removed: Set[str] = set()
        for term, info in list(self._terms.items()):
            new_postings = [p for p in info["postings"] if p["chunk_id"] not in to_remove]
            for p in info["postings"]:
                if p["chunk_id"] in to_remove:
                    removed.add(p["chunk_id"])
            if not new_postings:
                del self._terms[term]
            else:
                self._terms[term] = {"idf": info["idf"], "postings": new_postings}
        if not removed:
            return 0
        # 重新计算 N、avgdl（基于剩余 postings 中的 chunk）
